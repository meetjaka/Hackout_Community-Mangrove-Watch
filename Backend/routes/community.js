    ...
    res.json({
      ...
      data: {
        overview: {
          ...
          activeCommunities: community.activeMembers > 0 ? Math.ceil(community.activeMembers / 10) : 1,
    ...
    res.json({
      ...
      data: {
        ...
        filters: {
          ...
          available: {
            categories: [...new Set(community.resources.map(r => r.category))],
            difficulties: [...new Set(community.resources.map(r => r.difficulty))],
...
router.get('/events', async (req, res) => {
  try {
    const { upcoming = true, location, limit = 10, type, difficulty } = req.query;
...
router.get('/events/:id', async (req, res) => {
  try {
    ...
    const event = community.events.find(e => e._id.toString() === id && e.isActive);
    if (!event) {
      return res.status(404).json({
        success: false,
...
router.post('/events/:id/register', auth, async (req, res) => {
  try {
    ...
    const existingRegistration = event.registrations.find(r => r.userId.toString() === userId);
...
router.post('/events/:id/unregister', auth, async (req, res) => {
  try {
    ...
    const registrationIndex = event.registrations.findIndex(r => r.userId.toString() === userId);
  ...
  try {
    ...
    const participants = await Promise.all(
      event.registrations.map(async (registration) => {
        const user = await User.findById(registration.userId).select('firstName lastName avatar organization');
        return {
...
router.get('/resources/:id/interactions', async (req, res) => {
  try {
    ...
    const interactions = {
      ...
      userFavorited: userId ? (resource.favorites?.includes(userId) || false) : false
    };
...
router.get('/user/favorites', auth, async (req, res) => {
  try {
    ...
    const favoriteResources = community.resources.filter(r => 
      r.isActive && r.favorites?.includes(userId)
    );
